...
namespace ReadyConfig
{
    ...
    public class ExamplePlugin : BaseUnityPlugin
	{
        ...
        public void Awake()
        {
	        ...
	        EnableGameOverChanges = Config.Bind<bool>("GameOver", "EnableGameOverChanges", false, "Enable or Disable all features related to the GameOver screen");
	        GameOverRequiresAllReady = Config.Bind<bool>("GameOver", "GameOverRequiresAllReady", true, "Instead of starting the countdown when anyone has voted to skip, requires all Players to press vote");
	        GameOverCountdownTime = Config.Bind<float>("GameOver", "GameOverCountdownTime", 15f, "The countdown until skipped when not using GameOverRequiresAllReady, in seconds");
	        
	        EnableCreditsChanges = Config.Bind<bool>("Credits", "EnableCreditsChanges", true, "Enable or Disable all features related to the Credits screen");
	        CreditsCountdownOnAnyReady = Config.Bind<bool>("Credits", "CreditsCountdownOnAnyReady", true, "Instead of requiring everybody to vote, starts a countdown when at least one Player votes to skip");
	        CreditsCountdownTime = Config.Bind<float>("Credits", "CreditsCountdownTime", 15f, "The countdown until skipped when using CreditsCountdownOnAnyReady, in seconds");
	        
	        EnableLobbyChanges = Config.Bind<bool>("Lobby", "EnableLobbyChanges", true, "Enable or Disable all features related to the Lobby");
	        LobbyVotesRequireHostReady = Config.Bind<bool>("Lobby", "LobbyVotesRequireHostReady", false, "Players can't click Ready unless Host is Ready, disabled on dedicated servers");
	        LobbyStartOnHostReady = Config.Bind<bool>("Lobby", "LobbyStartOnHostReady", false, "Makes everyone Ready when Host is Ready and starts the game, disabled on dedicated servers");
	        LobbyStartRequiresAllReady = Config.Bind<bool>("Lobby", "LobbyStartRequiresAllReady", true, "Instead of starting the countdown when anyone has presses Ready, requires all Players to press Ready");
	        LobbyCountdownTime = Config.Bind<float>("Lobby", "LobbyCountdownTime", 60f, "The countdown until the game is started when not using LobbyStartRequiresAllReady, in seconds");
        ...
        private void PreGameControllerOnOnEnable(PreGameController.orig_OnEnable orig, RoR2.PreGameController self)
        {
	        ...
	        if (UnityEngine.Networking.NetworkServer.active)
	        {
		        ...
		        {
			        PreGameVoteController.timerStartCondition = RoR2.VoteController.TimerStartCondition.WhileAllVotesReceived;
		        }
	        }
        ...
        private void GameOverControllerOnOnEnable(GameOverController.orig_OnEnable orig, RoR2.GameOverController self)
        {
	        ...
	        if (UnityEngine.Networking.NetworkServer.active)
	        {
		        ...
		        {
			        GameOverVoteController.timerStartCondition = RoR2.VoteController.TimerStartCondition.WhileAllVotesReceived;
		        }
	        }
        ...
        private void CreditsControllerOnOnEnable(CreditsController.orig_OnEnable orig, RoR2.CreditsController self)
        {
	        ...
	        if (UnityEngine.Networking.NetworkServer.active)
	        {
		        ...
		        {
			        CreditsVoteController.timerStartCondition = RoR2.VoteController.TimerStartCondition.WhileAnyVoteReceived;
		        }
	        }
        ...
        private void LobbyVoteControllerOnReceiveUserVote(VoteController.orig_ReceiveUserVote orig, RoR2.VoteController self, NetworkUser networkUser, int voteChoiceIndex)
        {
	        Log.LogDebug(nameof(LobbyVoteControllerOnReceiveUserVote) + $" called by: {networkUser.userName}({networkUser.id.value}), Vote Choice Index: {voteChoiceIndex}, Host: {networkUser.isLocalPlayer}");
	        ...
	        if (!UnityEngine.Networking.NetworkServer.active)
	        {
		        Log.LogDebug("Not running on server, " + nameof(LobbyVoteControllerOnReceiveUserVote) + " cancelled, calling " + nameof(orig));
		        orig(self, networkUser, voteChoiceIndex);
		        return;
	        }
	        ...
	        {
		        Log.LogDebug(nameof(PreGameVoteController) + " doesn't equal " + nameof(self) + ", calling " + nameof(orig));
		        orig(self, networkUser, voteChoiceIndex);
		        return;
	        }
	        ...
	        else
	        {
		        if (LobbyVotesRequireHostReady.Value)
		        {
			        ...
			        {
				        if (NetworkUser.readOnlyInstancesList.Count > 0 && v.networkUserObject.Equals(NetworkUser.readOnlyInstancesList[0].gameObject) && v.voteChoiceIndex == 0)
				        {
					        orig(self, networkUser, voteChoiceIndex);
				        }
			        }
